...
class Opt():
    def __init__(self):
        ...
        self.id = ''
        self.train_only = 0
        self.pretrained_weight = None   ######### xud
...
opt.input_json = '/data/jiangqy/data_for_image_caption/20_20_2048/preprocessed_data/coco_ai_challenger_talk.json'
opt.input_fc_h5 = '/data/jiangqy/data_for_image_caption/20_20_2048/preprocessed_data/coco_ai_challenger_talk_resnet152_fc_wehnhao.h5'
opt.input_att_h5 = '/data/jiangqy/data_for_image_caption/jupyter/json_preprocess_data/coco_ai_challenger_talk_resnet151_att_.h5'
opt.input_label_h5 = '/data/jiangqy/data_for_image_caption/20_20_2048/preprocessed_data/coco_ai_challenger_talk_resnet152_label_wehnhao.h5'
...
#     print('calculate prediction')
    ...
    if iteration % iter_print_count == 0:
        print("iter {} (epoch {}), train_loss = {:.3f}, time/batch = {:.3f}"                 .format(iteration, epoch, sum(train_loss)/len(train_loss), end - start))
        train_loss = [] 
...
#                 tf_summary_writer.flush()
            ...
            if True: # if true
                ...
                with open(os.path.join(opt.checkpoint_path, 'histories_'+opt.id+'_attention_wehao.pkl'), 'wb') as f:
                    cPickle.dump(histories, f)

                if best_flag:
                    checkpoint_path = os.path.join(opt.checkpoint_path, 'model-best_attention_wehao.pth')
                    torch.save(model.state_dict(), checkpoint_path)
                    print("model saved to {}".format(checkpoint_path))
                    with open(os.path.join(opt.checkpoint_path, 'infos_'+opt.id+'-best_attention_wehao.pkl'), 'wb') as f:
                        cPickle.dump(infos, f) 
