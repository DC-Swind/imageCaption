...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    while True:
        ...
        labels = data['labels'][np.arange(loader.batch_size) * loader.seq_per_img]     #################
        #print('************ground_truth *************')
...
def eval_split_test(model, crit, loader, eval_kwargs={}):
    verbose = eval_kwargs.get('verbose', True)
    num_images = eval_kwargs.get('num_images', eval_kwargs.get('val_images_use', -1))
    split = eval_kwargs.get('split', 'val')
    lang_eval = eval_kwargs.get('language_eval', 0)
    ...
    while True:
        ...
        # forward the model to also get generated samples for each image
        # print(eval_kwargs)
        seq, log_prob = model.sample_0(fc_feats, att_feats, eval_kwargs)

        #set_trace()
        sents = utils.decode_sequence(loader.get_vocab(), seq)

        for k, sent in enumerate(sents):
            entry = {'image_id': data['infos'][k]['file_path'].split('/')[-1].split('.')[0],
                     'caption': sent.replace(' ',''),
                     'log_prob': float(np.exp(np.sum(log_prob[k].numpy())))}
            if eval_kwargs.get('dump_path', 0) == 1:
                entry['file_name'] = data['infos'][k]['file_path']
            predictions.append(entry)
            if eval_kwargs.get('dump_images', 0) == 1:
                ...
                # print(cmd)
                # os.system(cmd)
                print(data['infos'][k]['file_path'].split("/")[-1])
                pass

            if verbose:
                print('prob: ',entry['log_prob'])
                print('image %s: %s' %(str(k), entry['caption']))
