...
class DataLoader():
	...
	att_batch = np.ndarray((batch_size * self.seq_per_img, 14, 14, self.att_feat_size), dtype = 'float32')   ## Jie Zhang 14 -20
	##############  Jie Zhang
	...
	label_batch = np.zeros([batch_size * self.seq_per_img, self.seq_length + 2], dtype = 'int')
	    ...
	    # Jie Zhang:::::::fetch image
            ...
            att_batch[i* self.seq_per_img : (i + 1) * self.seq_per_img] = self.h5_att_file['att'][ix, :, :, :]
...
'''    def get_batch(self, split, batch_size=None, seq_per_img=None):
	...
	##############  Jie Zhang
	fc_batch_zj = np.ndarray((batch_size * self.seq_per_img, self.fc_feat_size), dtype = 'float32')
...
class DataLoader_pool(data.Dataset):
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        att_batch = [] # np.ndarray((batch_size * seq_per_img, 20, 20, self.opt.att_feat_size), dtype = 'float32')
