...
class DataLoader():
    ...
    def __init__(self, opt):
        ###
        self.exchange_vocab = False
        ###        
        self.type = 'h5'
        self.opt = opt
        self.att_im = 1 # getattr(opt, 'att_im', 1)
        # print(self.att_im)
        # print('********')
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
      ...
      #  print('batch_size',batch_size)
       ...
       # print('seq_per_img',seq_per_img)
        ### note: Jiang Qing-Yuan: change line 97 to 98 and line 99 to 100
        # fc_batch = np.ndarray((batch_size, self.fc_feat_size), dtype = 'float32') if self.pre_ft else None
        fc_batch = np.ndarray((batch_size, self.fc_feat_size* 18), dtype = 'float32')
        ...
        ##############  Jie Zhang
        fc_batch_zj = np.ndarray((batch_size * self.seq_per_img, self.fc_feat_size * 18), dtype = 'float32')
        att_batch_zj = np.ndarray((batch_size * self.seq_per_img, 14, 14, self.att_feat_size), dtype = 'float32')
        ##############  Jie Zhang
        ...
        for i in range(batch_size):
            ...
            # Jie Zhang:::::::fetch image
            fc_batch_zj[i* self.seq_per_img : (i + 1) * self.seq_per_img] = self.h5_fc_file['fc'][ix, :]
            att_batch_zj[i* self.seq_per_img : (i + 1) * self.seq_per_img] = self.h5_att_file['att'][ix, :, :, :]
        ...
        data = {}
        data['fc_feats'] = fc_batch_zj   ########
        data['att_feats'] = att_batch_zj  ########
        
        data['fc_feats_zj'] = fc_batch_zj
        data['att_feats_zj'] = att_batch_zj
...
class DataLoader_pool(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        # load the json file which contains additional information about the dataset
        print('DataLoader loading json file: ', opt.input_json)
        os.system('ls ' + self.opt.input_json)
        self.info = json.load(open(self.opt.input_json))
        self.ix_to_word = self.info['ix_to_word']
        self.vocab_size = len(self.ix_to_word)
        print('vocab size is ', self.vocab_size)
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        ##############  Jie Zhang
        fc_batch = np.ndarray((batch_size * self.seq_per_img, self.fc_feat_size * 18), dtype = 'float32')
        att_batch = np.ndarray((batch_size * self.seq_per_img, 14, 14, self.att_feat_size), dtype = 'float32')  ### jiezhang 14 -- 
	    ##############  Jie Zhang
        ...
        for i in range(batch_size):
            ...
            # Jie Zhang:::::::fetch image
            fc_batch[i* self.seq_per_img : (i + 1) * self.seq_per_img] = self.h5_fc_file['fc'][ix, :]
            att_batch[i* self.seq_per_img : (i + 1) * self.seq_per_img * 18] = self.h5_att_file['att'][ix, :, :, :]
