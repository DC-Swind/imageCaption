...
class DataLoaderRaw():
    ...
    def __init__(self, opt):
        ...
        # Load resnet
        self.cnn_model = 'resnet152' #opt.get('cnn_model', 'resnet101')
        self.my_resnet = getattr(misc.resnet, self.cnn_model)()
        self.my_resnet.load_state_dict(torch.load('./data/imagenet_weights/'+self.cnn_model+'.pth')) #'/home/jiangqy/PycharmProjects/Image_Caption/resnet/resnet152.pth'))#
        self.my_resnet = myResnet(self.my_resnet)
        self.my_resnet.cuda()
        self.my_resnet.eval()
        self.a =  h5py.File('/home/jiangqy/PycharmProjects/Multi_Label_Model_Python3/data/test_image_ids_hdf5.h5', "r")  ##qingyuan
        self.b =  h5py.File('/home/jiangqy/PycharmProjects/Multi_Label_Model_Python3/data/test_hdf5.h5', "r")##qingyuan
        print(self.my_resnet)
    ...
    def get_batch(self, split, batch_size=None):
        ...
        fc_batch = np.ndarray((batch_size, 2048*18), dtype = 'float32')#wenhao
        ...
        test_imge_folder = '/mnt/data3/zhengwh/dataset/rcnn_test_images'   ###
        for i in range(batch_size):  
            ...
            join_vector = np.zeros(2048*18) ###zj
            ...
            join_vector[: 2048] = tmp_fc.data.cpu().float().numpy()  ### 
            
            filename = (self.files[ri]).split('/')[-1]
            for num in range(17): 
                if os.path.isfile(os.path.join(test_imge_folder, (filename + '.' + str(num)+'.jpg'))) == True:
                    I = skimage.io.imread(os.path.join(test_imge_folder, (filename + '.' + str(num)+'.jpg')))
                else:
                    break 
                if len(I.shape) == 2:
                    I = I[:, :, np.newaxis]
                ...
                tmp_fc, tmp_att1 = self.my_resnet(I) 
                join_vector[(num+1)*2048 : 2048*(num+2)] = tmp_fc.data.cpu().float().numpy()   
            fc_batch[i] = join_vector 
            #att_batch[i] = tmp_att.data.cpu().float().numpy()
