...
def setup(opt):
    ...
    elif opt.caption_model == 'topdown_residual':
        model = TopDownModel_residual(opt)
    else:
    ...
    if vars(opt).get('start_from', None) is not None:
        ...
        assert os.path.isfile(os.path.join(opt.start_from,"infos_"+opt.id+"_attention.pkl")),"infos.pkl file does not exist in path %s"%opt.start_from
