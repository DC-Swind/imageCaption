...
class AttModel(nn.Module):
    def __init__(self, opt):
        ...
        self.drop_prob_lm_input = opt.drop_prob_lm_input   # -- xud
        ...
        #  ----- xud -------
        embedding_weight = nn.Embedding(self.vocab_size + 1, self.input_encoding_size) # index from 1
        if opt.pretrained_weight is not None:
            print("use pretrained embedding matrix.")
            # pretrained_weight is a numpy matrix of shape (num_embeddings, embedding_dim)
            embedding_weight.weight.data.copy_(torch.from_numpy(opt.pretrained_weight))

        self.embed = nn.Sequential(embedding_weight,
                                nn.Dropout(self.drop_prob_lm_input))
        ...
        self.ctx2att = nn.Linear(self.rnn_size, self.att_hid_size)  # W_va

    def init_hidden(self, bsz, fc_feats):
        weight = next(self.parameters()).data
        return (Variable(weight.new(self.num_layers, bsz, self.rnn_size).zero_()),
                Variable(weight.new(self.num_layers, bsz, self.rnn_size).zero_()), fc_feats)
    ...
    def forward(self, fc_feats, att_feats, seq):
        ...
        fc_feats = self.fc_embed(fc_feats)  # (100, 512)
        state = self.init_hidden(batch_size, fc_feats)
        _att_feats = self.att_embed(att_feats.view(-1, self.att_feat_size)) # (19600, 512)
        att_feats = _att_feats.view(*(att_feats.size()[:-1] + (self.rnn_size,))) # (100, 14, 14, 512)
    ...
    def sample_beam(self, fc_feats, att_feats, opt={}):
        ...
        for k in range(batch_size):
            ...
            state = self.init_hidden(beam_size, tmp_fc_feats)
            ...
            for t in range(self.seq_length + 1):
                if t == 0: # input <bos>
                    ...
                    for vix in range(beam_size):
                        ...
                        for state_ix in range(len(new_state)-1):
                            # copy over state in previous beam q to new beam at vix
                            new_state[state_ix][0, vix] = state[state_ix][0, v['q']] # dimension one is time step
                            new_state[state_ix][1, vix] = state[state_ix][1, v['q']]
                        new_state[2][vix] = state[2][v['q']]
    ...
    def sample(self, fc_feats, att_feats, opt={}):
        ...
        # embed fc and att feats
        fc_feats = self.fc_embed(fc_feats)
        state = self.init_hidden(batch_size, fc_feats)   #### xud
        _att_feats = self.att_embed(att_feats.view(-1, self.att_feat_size))
        att_feats = _att_feats.view(*(att_feats.size()[:-1] + (self.rnn_size,)))
    ...
    def sample_0(self, fc_feats, att_feats, opt={}):
        sample_max = opt.get('sample_max', 1)
        beam_size = opt.get('beam_size', 1)
        print(beam_size)
        temperature = opt.get('temperature', 1.0)
...
class TopDownCore(nn.Module):
    def __init__(self, opt, use_maxout=False):
        ...
        self.drop_prob_lm = opt.drop_prob_lm
        self.drop_prob_lm_input = opt.drop_prob_lm_input
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state):
        ...
        #att_lstm_input = torch.cat([prev_h, att_hat, xt], 1)
        ...
        h_lang = F.dropout(h_lang, self.drop_prob_lm, self.training)
        state = (torch.stack([h_att, h_lang]), torch.stack([c_att, c_lang]), att)
        ...
        #lang_lstm_input = torch.cat([att, h_att], 1)
        # the next line may be useful, applying dropout in vertical direction, but not in horizonal direction.
        lang_lstm_input = torch.cat([att, F.dropout(h_att, self.drop_prob_lm_input, self.training)], 1) #?????
...
#-----------------------------------------------------------------------
#------------------------------xud--------------------------------------
class TopDownCore_residual(nn.Module):
    def __init__(self, opt, use_maxout=False):
        super(TopDownCore_residual, self).__init__()
        self.drop_prob_lm = opt.drop_prob_lm

        self.att_lstm = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size) # we, fc, h^2_t-1
        #self.lang_lstm = nn.LSTMCell(opt.rnn_size * 2, opt.rnn_size) # h^1_t, \hat v
        #  ---- xud ---- for residual connection
        self.lang_lstm = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size) # we, h^1_t, \hat v
        #  -------------
...
class Attention(nn.Module):
    ...
    def forward(self, h, att_feats, p_att_feats):
        ...
        """
        #  ---  h^T W feature_i (general score function/ bilinear score function) ------
        att_h = self.h2att(h)                        # batch * att_hid_size
        att_h = att_h.unsqueeze(1).expand_as(att)            # batch * att_size * att_hid_size
        dot = att_h * att       # batch * att_size * att_hid_size
        # dot = tf.reduce_sum(dot, axis=2)    # batch * att_size
...
# --------------xud---------------------------
class TopDownModel_residual(AttModel):
    def __init__(self, opt):
        super(TopDownModel_residual, self).__init__(opt)
        self.num_layers = 2
        self.core = TopDownCore_residual(opt)
#---------------xud----------------------------
